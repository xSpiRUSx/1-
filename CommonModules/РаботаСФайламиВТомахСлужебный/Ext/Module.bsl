...
Функция ПолноеИмяФайлаВТоме(СвойстваФайла, ДатаДляРазмещенияВТоме = Неопределено) Экспорт
	...
	Если ТипЗнч(СвойстваФайла.ВладелецФайла) = Тип("ДокументСсылка.ЗаказНаРаботу") Тогда
		
		Конфигурация = СвойстваФайла.ВладелецФайла.Конфигурация.Наименование;
		
		Если НЕ ЗначениеЗаполнено(Конфигурация) Тогда
			Конфигурация = "Не заполнено";
		КонецЕсли;
		
		КорневойКаталог = КорневойКаталог + Конфигурация + Разделитель;
