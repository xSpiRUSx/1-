...
Процедура УстановитьТекст(Текст, Позиция, УчитыватьОтступПервойСтроки)
	...
	View().updateText(Текст, Позиция, УчитыватьОтступПервойСтроки);
...
Функция ПолучитьМакет(ИмяМакета)
	...
	Возврат Справочники.Информация.ПолучитьМакет(ИмяМакета);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Иначе
		АдресМетаданных = Справочники.Информация.ЗаполнитьКоллекциюМетаданных();
	//КонецЕсли;
	//
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Список.ТекстКода = ПолучитьТекст();
КонецПроцедуры
...
Процедура ОбъектПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Элементы.ПолеHTML.Документ.defaultView.setReadOnly(true);
		УстановитьТекст(ТекущиеДанные.ТекстКода, Неопределено, Ложь);		
